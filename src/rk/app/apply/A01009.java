...
public class A01009 extends JGAction{
	
	protected final String _querysetKeyName = "app.apply.A01009";
	
	public int canLeaderApply(JGServiceBox serviceBox_, String misId_) throws Exception{
		String memSid_ = RKSessionData.currentMemSid(serviceBox_);
		JGDBQuery query_ = JGDBXMLQueryManager.sharedManager().createQuery(_querysetKeyName, "canLeaderApply"
				,new Object[]{"misId",misId_,"memSid",memSid_});
		return (getDBConnection().executeQueryAndGetFirst(query_).equals("Y") ? 0 : -1);
	}
	
	public int decodeNewLeaderApply(JGServiceBox serviceBox_) throws Exception{
		String misId_ = serviceBox_.getParameter("misId");
		String memSid_ = RKSessionData.currentMemSid(serviceBox_);
		
		if(misId_ == null) return -2;
		
		JGDBQuery query_ = JGDBXMLQueryManager.sharedManager().createQuery(_querysetKeyName, "checkLeaderApply"
				,new Object[]{"misId",misId_,"memSid",memSid_});
		JGDataset result_ = getDBConnection().executeQuery(query_);
		
		if(result_.getRowCount() > 0) return -1;
		if(canLeaderApply(serviceBox_, misId_) == -1) return -2;
		
		query_ = JGDBXMLQueryManager.sharedManager().createQuery(_querysetKeyName, "initializeLeaderApplyData"
				,new Object[]{"misId",misId_,"memSid",memSid_});
	...
	public void updateApplyData(JGServiceBox serviceBox_) throws Exception{
		...
		getDBConnection().executeUpdate(basicData_, "AT_APPLY_L_BSC");
		getDBConnection().executeUpdate(qnaData_, "AT_APPLY_L_QNA");
		getDBConnection().commit();
