$(document).on("pagebeforecreate",function(){
	var condData_ = JGDS("dataset","a01001CondData");
	var condDataView_ = $("[jg-dataset='a01001CondData']");
	...
	var misView_ = $("[jg-dataset='a01001MinistryList']");
...
$(document).ready(function(){
	...
	RKCommon.ministry.getMinistryStatusList(function(data_){
		var dataset_ = JGDS("dataset","a01001StatusData",data_);
	...
	RKCommon.ministry.getMinistryYearList(function(data_){
		var dataset_ = JGDS("dataset","a01001YearData",data_);
		dataset_.insertRow(0);
		dataset_.setColumnValues({
			ID : ""
			,NM : "사역년도"
...
function atFuncA01001_search(){
	var condData_ = JGDS("dataset","a01001CondData");
	var misData_ = JGDS("dataset","a01001MinistryList");
	JGService.ajax(RKCommon.apply.requestURLKey,{
		data : {
			srvID : "getMemberApplyTargetList"
			,condData : condData_.toJSONString(false)
		},success : function(data_){
			misData_.applyJSON(data_.message);
			$("#a01001").attr("rk-content-list-count",misData_.getRowCount());
		},error : function(){
			alert("에러발생!");
		}
	});
}
function atFuncA01001_applyHandle(rowIndex_){
	var misData_ = JGDS("dataset","a01001MinistryList");
	var misId_ = misData_.getColumnValue("mis_id",rowIndex_);
	var didWrite_ = misData_.getColumnValue("did_write",rowIndex_) === "Y";
	
	if(!didWrite_){
		location.href = JGService.requestURL(RKCommon.apply.requestURLKey,{srvID : "newMemberApplyView"
			,misId : misId_});
	}else{
